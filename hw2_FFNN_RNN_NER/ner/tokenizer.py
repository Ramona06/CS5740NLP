...
class Tokenizer(object):
    ...
    def tokenize(
        ...
        # Split input_seq if it's a string
        if isinstance(input_seq, str):
            input_seq = input_seq.split()

        # Check case if not lowercase, set to 'UNK_TOKEN' if needed
        if not self.lowercase:
            input_seq = [token if token in self.token2id else UNK_TOKEN for token in input_seq]

        # Get a list of the corresponding IDs based on the dict token2id
        input_ids = [self.token2id.get(token, self.token2id[UNK_TOKEN]) for token in input_seq]

        # Padding and truncation
        if max_length:
            # If less than max_length, then padding
            if len(input_seq) < max_length:
                num_padding_tokens = max_length - len(input_seq)
                if padding_side == 'right':
                    input_seq.extend([self.pad_token] * num_padding_tokens)
                    #print('padded tokens:',input_seq)
                else:
                    input_seq = [self.pad_token] * num_padding_tokens + input_seq
            # If longer than max_length, then truncate
            if len(input_seq) > max_length:
