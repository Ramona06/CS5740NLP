...
class MultiHeadAttention(Module):
    ...
    def masked_attention_probs(
        ...
        #print(query.shape) #batch, num_heads, max_length, head (b,#h, i, h)
        #print(key.shape) # #batch, num_heads, max_length, head (b,#h, j, h)
        # while head_dim canceled out to be a constant with the "dot product"
        # the shape of the similarity from the function is (b, #h, i*j)
        similarities = torch.einsum('...ik,...jk->...ij', query, key)
        similarities = similarities/np.sqrt(query.shape[3]) #a.k.a. Attention
        ...
        similarities = softmax(self._apply_causal_mask(similarities), dim=3, dropout_proba = self.attn_dropout_proba, training = self.training)
    ...
    def attention(masked_attn_probs: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        ...
        att = torch.einsum("...ij,...jk->...ik",masked_attn_probs,value)
