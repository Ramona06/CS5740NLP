...
class SeagullLM(Module):
    ...
    def forward(
        ...
        # TODO-6.2
        transformerLayers = self.seagull(input_ids, position_ids, padding_mask, use_kv_cache, return_output_at_all_layers, return_attentions)
        finalTransformerLayer = transformerLayers[-1]
        
        # print(finalTransformerLayer.shape)
        if self.weight_tying:
          weights = torch.transpose(self.seagull.embedding.token_embedding.weight.data, 0 , 1)
          finalTransformerLayer = torch.matmul(finalTransformerLayer, weights)
        else:
          finalTransformerLayer = self.lm_head(finalTransformerLayer)
        return finalTransformerLayer, transformerLayers
